...
extractGeneName <- function(row) {
    ...
    s <- str_trim(str_sub(s, start = gene_name[1], end = gene_name[2]))
...
extractGeneId <- function(row) {
    ...
    #   print(gene_id)
    s <- str_trim(str_sub(s, start = gene_id[1], end = gene_id[2]))
...
extractGeneBiotype <- function(row) {
    ...
    s <- str_trim(str_sub(s, start = gene_biotype[1], end = gene_biotype[2]))
...
findFeatureForCmhSnps <- function(featureFileName, gwasFileName, outputPrefix, outputDirPath) {
    ...
    dbName <- paste(outputDirPath,outputPrefix,"_SNPS_IN_FEATURES_DATA",".sqlite", sep="")
...
gtfDirPath = "/home/nehil/data_files/"
gwasDirPath = "/share/volatile_scratch/nehil/snp-cmh-igv-files/"
outputDirPath <- "/share/volatile_scratch/nehil/gene-analysis/"
