...
manhattan <- function(dataframe, limitchromosomes=NULL,pt.col=c('blue2','aquamarine2','darkgoldenrod1','coral1'),pt.bg=c('black'),
    ...
    suggestiveline=-log10(1e-5), suggestiveline.col='white', suggestiveline.lwd=1.5, suggestiveline.lty=1, 
    genomewideline=-log10(5e-8), genomewideline.col='white', genomewideline.lwd=1.5, genomewideline.lty=1, 
    ...
    d=subset(d[order(d$CHR, d$BP), ], (P>0 & P<=4)) # sort, and keep only 0<P<=1
    d$logp = -log10(d$P)
    d$pos=NA
    ...
    plot(0,col=F,xaxt='n',bty='n',xaxs='i',yaxs='i',xlim=c(xmin,xmax), ylim=c(ymin,ymax),
            xlab=xlabel,ylab="LHSNP",las=1,cex.axis=cex.axis)
...
qq = function(pvector,gridlines=F,gridlines.col='gray83',gridlines.lwd=1,gridlines.lty=1,confidence=T,confidence.col='gray81',
    pt.cex=0.5,pt.col='black',pt.bg='black',pch=21,abline.col='white',abline.lwd=1.8,abline.lty=1,ymax=8,ymax.soft=T,
    highlight=NULL,highlight.col=c('green3','magenta'),highlight.bg=c('green3','magenta'),
    ...
    #ymax = ceiling(ymax * 1.03)
    ymin = -ymax*0.03
    plot(0,xlab=expression(Expected~~-log[10](italic(p))),ylab="LHGV",
