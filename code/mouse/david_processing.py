...
geneList = pd.read_csv('../'+kAnalysisName+'/Data/ProcessedData/top_genelist_for_wgr.csv')

print 'length of the Ensemble gene list = ', len(geneList)

testGeneData = geneList[0:200]

funcPathWayData = pd.read_table('../'+kAnalysisName+'/Data/david_functional_annotation_chart_report.txt', sep = '\t')

print 'length of the Ensemble gene list = ', len(funcPathWayData)

funcPathWayData = funcPathWayData.drop(['Count', '%', 'List Total', 'Pop Hits','Pop Total','Fold Enrichment','Bonferroni','Benjamini','FDR'], axis = 1)


textFilename = '../'+kAnalysisName+'/Data/david_cluster_report.txt'


clusterData = list(csv.reader(open(textFilename, 'rb'), delimiter = '\t'))
print 'length of the cluster list = ', len(clusterData)
...
for geneIndex, geneRow in geneList.iterrows():
    tempGeneRow = [ geneRow['x'], [], [], 0.0,  'NAN', 0,  0, 'NAN', 'NAN', 0, 0.0, 0, '', ''  ]
    weightedGeneList.append(tempGeneRow)
    masterGeneList.append(geneRow['x'])

testFuncData = funcPathWayData[0:10]
...
for funcPathIndex, funcPathRow in funcPathWayData.iterrows():
...
for geneRow in weightedGeneList:
    geneRow[11] = len(geneRow[1] + geneRow[2])
